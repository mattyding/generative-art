...
def update_lines(num_steps, walks, lines, pause_threshold):
    for walk, line in zip(walks, lines):
        if num_steps < pause_threshold:
            line.set_data(walk[:num_steps * FRAME_SKIP, 0],
                          walk[:num_steps * FRAME_SKIP, 1])
        else:  # pause animation at end
            line.set_data(walk[:pause_threshold * FRAME_SKIP, 0],
                          walk[:pause_threshold * FRAME_SKIP, 1])
    return lines
...
def random_walk_animation(params, walks, cmap, seed):
    ...
    fig, ax = plt.subplots()
    lines = [ax.plot(walk[0], walk[1], color=cmap(i))[0]
    ...
    anim = animation.FuncAnimation(
        ...
        interval=100)
    plt.savefig(
        f"./figures/randomwalk/{seed}-{num_steps}-{max_step_size}-{num_walks}/walk-full.png",
        dpi=500)
    writergif = animation.PillowWriter(fps=20)
    anim.save(
        f"./figures/randomwalk/{seed}-{num_steps}-{max_step_size}-{num_walks}/walk.gif",
        writer=writergif)

    fig.savefig(
        f"./figures/randomwalk/{seed}-{num_steps}-{max_step_size}-{num_walks}/walk-small.png",
        dpi=500)
...
def random_walk_stats_animation(params, walks, cmap, seed):
    ...
    expectations = [(np.mean([walk[i][0] for walk in walks]), np.mean(
        [walk[i][1] for walk in walks])) for i in range(len(walks[0]))]
    expectations_x = [expectation[0] for expectation in expectations]
    expectations_y = [expectation[1] for expectation in expectations]
    variances = [np.mean([(expectations[i] - walk[i]) ** 2 for walk in walks])
    ...
    walk_plot.axis('off')
    lines = [walk_plot.plot(walk[0], walk[1], color=cmap(i))[0]
             for i, walk in enumerate(walks)]
    ...
    expectation_line = expectation_plot.plot(
        expectations[0][0], expectations[0][1], color='black')[0]
    ...
    def update_graph(num_steps, walks, lines, pause_threshold):
        ...
        if num_steps < pause_threshold:
            expectation_line.set_data(
                expectations_x[:num_steps * FRAME_SKIP], expectations_y[:num_steps * FRAME_SKIP])
            if num_steps > 1:
                # 0.1 buffer from edges
                expectation_plot.set_xlim(min(expectations_x[:num_steps * FRAME_SKIP]) - 0.1, max(
                    expectations_x[:num_steps * FRAME_SKIP]) + 0.1)
                expectation_plot.set_ylim(min(expectations_y[:num_steps * FRAME_SKIP]) - 0.1, max(
                    expectations_y[:num_steps * FRAME_SKIP]) + 0.1)
                variance_plot.plot(
                    variances[:num_steps * FRAME_SKIP], color='black')
        else:
            expectation_line.set_data(
                expectations_x[:pause_threshold * FRAME_SKIP], expectations_y[:pause_threshold * FRAME_SKIP])
            if num_steps > 1:
                # 0.1 buffer from edges
                expectation_plot.set_xlim(min(expectations_x[:pause_threshold * FRAME_SKIP]) - 0.1, max(
                    expectations_x[:pause_threshold * FRAME_SKIP]) + 0.1)
                expectation_plot.set_ylim(min(expectations_y[:pause_threshold * FRAME_SKIP]) - 0.1, max(
                    expectations_y[:pause_threshold * FRAME_SKIP]) + 0.1)
                variance_plot.plot(
                    variances[:pause_threshold * FRAME_SKIP], color='black')
        return lines
    ...
    writergif = animation.PillowWriter(fps=20)
    anim.save(
        f"./figures/randomwalk/{seed}-{num_steps}-{max_step_size}-{num_walks}/stats.gif",
        writer=writergif)
...
def random_walk_art(num_steps=400, max_step_size=0.15, num_walks=50):
    ...
    np.random.seed(seed)
    os.mkdir(
        f"{os.getcwd()}/figures/randomwalk/{seed}-{num_steps}-{max_step_size}-{num_walks}")
    walks = [random_walk(num_steps, max_step_size) for _ in range(num_walks)]
    cmap = get_cmap(num_walks)
    random_walk_animation(
        (num_steps, max_step_size, num_walks), walks, cmap, seed)
    random_walk_stats_animation(
        (num_steps, max_step_size, num_walks), walks, cmap, seed)
